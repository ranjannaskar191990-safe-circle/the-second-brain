...
const taskIntent=/\\b(remind me|todo|to-do|need to|must|have to|to be done|schedule)\\b|মনে করিয়ে|করতে হবে|করা দরকার|করবো|করব|করতে হবে|करना है|करना होगा|याद दिलाना/i;
...
function taskInfo(t){
 ...
 if(/আগামীকাল|কালকে|কাল|tomorrow|कल/.test(s)){d.setDate(d.getDate()+1);dueDate=[d.getFullYear(),String(d.getMonth()+1).padStart(2,"0"),String(d.getDate()).padStart(2,"0")].join("-")}
 else if(/পরশু|day after tomorrow/.test(s)){d.setDate(d.getDate()+2);dueDate=[d.getFullYear(),String(d.getMonth()+1).padStart(2,"0"),String(d.getDate()).padStart(2,"0")].join("-")}
 const dm=t.match(/(?:^|\\s)(\\d{1,2})(?:st|nd|rd|th)?\\s*(?:তারিখ|date)(?:\\s|$)/i);if(dm){const x=new Date(now);x.setDate(+dm[1]);if(x<now)x.setMonth(x.getMonth()+1);dueDate=[x.getFullYear(),String(x.getMonth()+1).padStart(2,"0"),String(x.getDate()).padStart(2,"0")].join("-")}
 let dueTime="";const tm=t.match(/(?:at\\s*)?(\\d{1,2})(?::(\\d{2}))?\\s*(am|pm|টা|টায়|টায়)?/i);if(tm&&(/am|pm|টা|টায়|টায়/i.test(tm[3]||""))){let h=+tm[1],m=+(tm[2]||0);if((tm[3]||"").toLowerCase()==="pm"&&h<12)h+=12;if((tm[3]||"").toLowerCase()==="am"&&h===12)h=0;dueTime=String(h).padStart(2,"0")+":"+String(m).padStart(2,"0")}
