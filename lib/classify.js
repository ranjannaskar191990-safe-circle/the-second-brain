...
function taskInfo(t){
 ...
 if(!dueTime){const tm=t.match(/(?:at\s*)?(\d{1,2})(?::(\d{2}))?\s*(a\.?\s*m\.?|p\.?\s*m\.?|টা|টায়|টায়)/i);if(tm){let h=+tm[1],m=+(tm[2]||0),p=(tm[3]||"").toLowerCase().replace(/[.\s]/g,"");if((p==="pm"||(/বিকেল|সন্ধ্যা|সন্ধ্যে|রাত/.test(t)&&h<12))&&h<12)h+=12;if(p==="am"&&h===12)h=0;dueTime=String(h).padStart(2,"0")+":"+String(m).padStart(2,"0")}}
...
export function classify(text){
 ...
 return{original:raw,normalized:t,language,category,amount,...fi,...ti,kmFrom:km?.[1]?.replace(/\s/g,"")||"",kmTo:km?.[2]?.replace(/\s/g,"")||"",workType,status:"New"};
}
