...
const finance=/₹|\brs\.?\b|inr|টাকা|रुपये|spent|spend|খরচ|खर्च|breakfast|lunch|dinner|expense/i;
...
export function classify(text){
 ...
 const km=t.match(kmRe),nums=[...t.matchAll(/(?:₹|rs\.?|inr)?\s*(\d+(?:\.\d{1,2})?)\s*(?:টাকা|रुपये|rupees?|rs\.?|₹)/ig)];
 ...
 return{original:t,language,category,amount,kmFrom:km?.[1]?.replace(/\s/g,"")||"",kmTo:km?.[2]?.replace(/\s/g,"")||"",workType,status:"New"};
}